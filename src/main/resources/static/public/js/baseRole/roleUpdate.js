layui.use(['form','layer'],function(){
    	...
    	//弹框
    	 layui.layer.open({
    	        ...
    	        ,yes: function(){
    	        	...
    	        	//设置权限输入框和权限id输入框的值，一个用于展示，一个用于传递
    	        	$('#auths').val(names);
    	        	$('#mId').val(ids);
    	        	//关闭
	...
	$(document).ready(function(){
		$.ajax({
			type:"post",
			url:"baseMenu/listByRole.do",
			data:{"roleId":$('#roleId').val()},
			dataType:"json",
			//通过角色id拿到所有权限信息，对于该角色拥有的权限，其checked字段值为true，角色没有的为false，zTree会自动识别其checked属性，若为true则会被默认选中
